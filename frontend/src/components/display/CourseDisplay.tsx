...
export const CourseDisplay = ({ courses, setCourses, fetchCourses }: CourseDisplayProps) => {
    ...
    async function unregisterSection(section: SectionItem) {
        api.deleteRegisteredSection(section.crn)
        setRegisteredSections(prev => prev.filter(sec => sec.crn !== section.crn))
    }
    ...
    return (
        <div className="h-[calc(100vh-48px)] overflow-hidden flex flex-row">
            ...
            <div className="overflow-y-auto flex flex-col w-full">
                <Course course={currentCourse} setCourse={setCurrentCourse} registerSection={registerSection} fall={fall} spring={spring} />
            </div>

            <div className="flex flex-row justify-left">
                <div className="flex justify-around w-[300px] bg-neutral-primary flex-shrink-0">
                    <RegisteredSections
            ...
            <Toaster toasterId={cannotRegisterToast} />
        </div>
    )
}